#include "NotImprovedPoseEstimator.h"
#include <opencv2/opencv.hpp>

using namespace std;
using namespace cv;

NotImprovedPoseEstimator::NotImprovedPoseEstimator(const Camera& camera, string imageFile, double imageWidth) :
...
Pose NotImprovedPoseEstimator::estimatePose(const Mat& img) {
	Pose pose;
	pose.valid = false;
...
Pose NotImprovedPoseEstimator::estimatePoseHomo(const Mat& img) {
	Pose pose;
	pose.valid = false;
	...
	// estimate relative pose of camera positions
	Pose relativePose;
	relativePose.valid = false;
	if (goodPoints2D.size() > 20) {
		if (solvePnPRansac(goodPoints3D, goodPoints2D, camera.K, camera.d, relativePose.rvec, relativePose.tvec)) {
			relativePose.valid = true;
		}
	}


	// adjust the pose to the world frame
	Pose cameraPose = prevPose;
	Pose absolutePose;
	if (cameraPose.valid && relativePose.valid) {
		//
	}

	//recoverPose(E, goodPoints2D, goodPoints2D, camera.K, camera.d, relativePose.rvec, relativePose.tvec);
